...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    print spath
    y,x = zip(*spath)
    plt.plot(x,len(grid)-1-plt.array(y),'o-')
    y,x = zip(*gxy)
    plt.plot(x,len(grid)-1-plt.array(y),'x',color='r')
    mvs=[]
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ex,ey,_ = estimate
        sx,sy = spath[index]
        dx,dy = spath[index+1][0] - sx, spath[index+1][1] - sy
        rx,ry = ex - sx, ey - sy
        cte = (ry*dx - rx*dy)/(dx**2 + dy**2)
        u = (rx*dx + ry*dy)/(dx**2 + dy**2)
        print index, spath[index], cte
        ...
        #if printflag:
        print myrobot, cte, index, u, myrobot.check_goal(goal) 
        mvs.append((myrobot.x,myrobot.y))

    y,x = zip(*mvs)
    plt.plot(x,len(grid)-1-plt.array(y),'o',color='g',alpha=0.2)
    plt.show()
