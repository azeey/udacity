...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        #Omega.show('Omega3')
        #Xi.show('Xi3')
        # Fold the matrix to keep useful information
...
import pylab as plt
poses = plt.array(result.value)[0:2*N]
lm = plt.array(result.value)[2*N:]
r = poses.reshape(N,2)
x,y = r.T
plt.scatter(x,y)
x,y = lm.reshape(len(lm)/2,2).T
plt.scatter(x,y,s=500,alpha=0.1,c='g')
...
#online_slam(data, N, num_landmarks, motion_noise, measurement_noise)
print_result(1, num_landmarks, result[0])
poses = plt.array(result[0].value)[0:2]
lm = plt.array(result[0].value)[2:]

x,y = poses.reshape(len(poses)/2,2).T
plt.scatter(x,y,c='r',alpha=.5,s=300)
x,y = lm.reshape(len(lm)/2,2).T
plt.scatter(x,y,c='y',alpha=.5,s=300)
plt.show()
