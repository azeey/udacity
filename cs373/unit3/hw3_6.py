...
class robot:
    ...
    def move(self, motion):
        ...
        b_calc = d/self.length * tan(motion[0])

        b = random.gauss(b_calc,self.steering_noise)

        if abs(b) > 0.001:
            r = d/b
            cx = self.x - r*sin(self.orientation)
            cy = self.y + r*cos(self.orientation)
            x = cx + r * sin(b + self.orientation)
            y = cy - r * cos(b + self.orientation)
    ...
    def sense(self,have_noise=1):
        ...
        for l in landmarks:
            ...
            # Add noise
            if have_noise==0:
                b += random.gauss(0, self.bearing_noise)
            Z.append(b)
...
#    print estimated_position[:2]
#    locs.append(estimated_position[:2])

#lx,ly = zip(*locs)
#plt.scatter(lx,ly,s=40)
#final_robot = robot()
#final_robot.set(93.476,75.186,5.2664)
#plt.fill([final_robot.x-tolerance_xy/2,final_robot.x+tolerance_xy/2,final_robot.x+tolerance_xy/2,final_robot.x-tolerance_xy/2],
#         [final_robot.y-tolerance_xy/2,final_robot.y-tolerance_xy/2,final_robot.y+tolerance_xy/2,final_robot.y+tolerance_xy/2],'g',alpha=0.2)

#plt.scatter(final_robot.x,final_robot.y,s=100,c='r',alpha=0.4)
#plt.xlim(0,100)
#plt.ylim(0,100)
#plt.grid()
#plt.show()
...
for i in range(M):
    estimated_position = particle_filter(motions, measurements)
    locs.append(estimated_position[:2])
    rc = check_output(final_robot, estimated_position)
    if rc == True: count +=1
...
print "Percentage: %f%%" % (count*100./M)

lx,ly = zip(*locs)
plt.scatter(lx,ly,s=40)
plt.fill([final_robot.x-tolerance_xy,final_robot.x+tolerance_xy,final_robot.x+tolerance_xy,final_robot.x-tolerance_xy],
        [final_robot.y-tolerance_xy,final_robot.y-tolerance_xy,final_robot.y+tolerance_xy,final_robot.y+tolerance_xy],'g',alpha=0.2)
